<x-frontend-layout>
     ...
     <div class="flex flex-col items-center mt-12 text-center">
       <span class="relative inline-flex w-full md:w-auto">
         <a href="{{ route('reservation.stepone') }}" type="button"
   ...
   <section class="mt-8 bg-white">
     ...
     <div class="container w-full px-5 py-6 mx-auto">
       <div class="grid lg:grid-cols-4 gap-y-6">
            @if(isset($special_meals))
                @foreach ($special_meals->menus as $special_meal)
                    <div class="max-w-xs mx-4 mb-2 rounded-lg shadow-lg">
                        <img class="w-full h-48" src="{{ asset('/').$special_meal->image }}"
                        alt="Image" />
                        <div class="px-6 py-4">
                            <h4 class="mb-3 text-xl font-semibold tracking-tight text-green-600 uppercase">{{ $special_meal->name }}</h4>
                            <p class="leading-normal text-gray-700">{{ $special_meal->description }}</p>
                        </div>
                        <div class="px-6 py-4">
                            <div class="flex items-center justify-between">
                                <span class="text-xl text-green-600">{{ $special_meal->price }}</span>
                            </div>
                        </div>
                    </div>
                @endforeach
