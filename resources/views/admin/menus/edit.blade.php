                ...
                <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                    <form method="POST" action="{{ route('admin.menus.update', $menu->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="sm:col-span-6 pt-5">
                            ...
                            <div class="mt-1">
                                <input type="number" value="{{ $menu->price }}" min="0.00" max="10000.00" step="0.01" id="price" wire:model.lazy="price" name="price" class="block w-full transition duration-150 ease-in-out appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5  @error('price') border-red-400 @enderror"/>
                                @error('price')
                                    <div class="text-sm text-red-400">{{ $message }}</div>
                                @enderror
                            </div>
